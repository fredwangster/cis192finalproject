...
class Crawler():
    ...
    def crawler(self, root, crawl_depth):
        ...
        #crawl not visited sites. new sites added through scrape_urls()
        while crawl_depth > 0:
        ...
        print "Time elapsed: %s secs, URLS: %s links" % \
              (end - start, len(self.visited))
    ...
    def scrape_urls(self, root, url_name):
        '''finds links in a given url returns the new_urls found'''
        try:
            ...
            for link in soup.find_all('a'):
                ...
                if (raw_url is not None) and (raw_url != "") and \
                   (raw_url not in self.to_visit) and \
                   (raw_url not in self.visited):
                    #don't add foreign urls
                    if 'http' in raw_url and (root not in raw_url):
                        new_urls.add("")
                    elif not ('http' in raw_url):
                        #http isn't in file name (rel url), so we must
                        #create it ourselves
